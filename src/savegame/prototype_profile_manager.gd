...
func _add_ranks(value: String) -> String:
	if !value.is_valid_int():
		return "[color=red]Argument was not a valid int[/color]"

	GlobalSaveGameManager.add_ranks(int(value))
	GlobalSaveGameManager.save_profile(current_profile)
	return _get_ranks()
	
func _get_ranks() -> String:
	return "Currently %s ranks collected" % current_profile.save_game.meta_progression.ranks
...
func _register_commands():
	...
	Console.register_custom_command("add_ranks", _add_ranks, ["(int) ranks to add or remove"], "Add ranks to player profile")
	Console.register_custom_command("get_ranks", _get_ranks, [], "Get the current amount of ranks")
...
func _unregister_commands():
	...
	Console.remove_command("add_ranks")
	Console.remove_command("get_ranks")
