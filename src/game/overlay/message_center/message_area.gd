...
func add_simple_message(target: int, message: String, time_to_show: float):
	add_new_message(target, message, time_to_show)

func add_new_message(target: int, message: String, time_to_show: float, message_icon: Texture = null):
	...
	message_target.add_new_message(message, time_to_show, message_icon)
